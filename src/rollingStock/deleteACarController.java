...
public class deleteACarController implements Initializable {
    ...
    ObservableList<deleteACarController.Locals> list = FXCollections.observableArrayList();
    String deleteRecord;
        ...
        if (rs != null) {
            try {
                while (rs.next()){
                    ...
                    list.add(new deleteACarController.Locals(roadName1, roadNumber1, type1, color1, owner1, rfid1) );
                }
            } catch (SQLException ex) {
                Logger.getLogger(deleteACarController.class.getName()).log(Level.SEVERE, null, ex);
            }
        }
    ...
    private void tableViewMouseAction(MouseEvent event) {
        ...
        try{
            deleteACarController.Locals item = tableView.getSelectionModel().getSelectedItem();
